...
class CosThetaInitialSplashScreen(QSplashScreen):
    ...
    def show(self) -> None:
        ...
        super().show()
        super().showMessage("<h1><font color='green'>Loading and preparing libraries!</font></h1>", Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignHCenter,
                           Qt.GlobalColor.black)
        sleepTime = 0.1
        timeDelay = int(self._delayTime / sleepTime)
        for i in range(timeDelay):
            time.sleep(sleepTime)
            self.progressBar.setValue(int(i * 100 / timeDelay))
            app.processEvents()
